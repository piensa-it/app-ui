...
- La aplicación de ejemplo enseña ahora los indicadores con icono y tono, mostrados u ocultos según su módulo de configuración técnica (no por cada persona: `StatGroup` no trae un botón para ocultarlos a propósito); una columna de acciones por fila con iconos (ver, editar, anular) con su nombre en el tooltip y en el nombre accesible; y el buscador en la barra superior. `DataTable` tiene la story «Con acciones por fila».
