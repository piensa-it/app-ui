...
- **`SidebarSearch`: el buscador de opciones del menú.** Un campo arriba de la navegación con la lupa, limpiar y Escape, con los tokens del menú; con el menú plegado no se pinta. La librería no conoce los enlaces: la aplicación filtra los suyos con el texto (`normalizeSearch` ignora tildes y mayúsculas), como hacen los ejemplos, ocultando las secciones sin resultados.
- **`Stat` con el icono en una tesela por tono**, arriba a la derecha, con el lenguaje de `IconTile`: un icono de 16 px junto al rótulo no se veía.
