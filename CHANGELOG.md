...
- **`TopbarIdentity`: sistema, empresa y módulo en la barra superior** (#119). La identidad estaba repartida: la empresa en la cabecera del menú, el nombre del sistema en un texto suelto de `topbarStart` y el módulo, en dos niveles, en el riel. Ahora una línea con hasta tres segmentos en `topbarStart`: el sistema (nombre y logo), la empresa y el módulo, cada uno un menú con marca de selección y descripción (el mismo de `SidebarBrand`) o un desvío por `onSelect` a un `AppSwitcher` con confirmación; el distintivo de entorno sale de la opción elegida. El módulo es opcional y sin él no queda hueco. La empresa se cambia en un solo sitio: con esto puesto, `SidebarBrand` va sin el grupo de empresa. Las cinco aplicaciones de ejemplo lo usan.
- **Enmarcado: `layout="framed"`** (#118). El menú encierra el contenido, como en la plantilla 2 de la evaluación: la raíz del armazón es del color del menú, el menú pierde el borde, y la página —barra superior incluida— va dentro de un panel redondeado sobre `ground` con un paso de espaciado alrededor. El panel es quien se desplaza, no la ventana, así el marco no se mueve nunca y la barra queda pegada a su borde. En móvil no hay marco. Se combina con `sidebarTone`, `SidebarProfile` y `topbarCenter`; en oscuro con el estilo `deep` y la paleta cian es esa plantilla. «Aplicación de ejemplo 5 · enmarcada» en Guías.
