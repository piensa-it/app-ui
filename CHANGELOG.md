...
- **`SidebarIdentity`: sistema, compañía y módulo en la cabecera del menú** (#119). La identidad estaba repartida: la compañía en `SidebarBrand`, el nombre del sistema en un texto suelto de la barra y el módulo, en dos niveles, en el riel. Ahora un bloque en el hueco `brand` con el sistema (marca y nombre), la compañía —con su distintivo de entorno, porque el sistema es el mismo y los ambientes se paralelizan por compañía— y el módulo, cada uno un menú con marca de selección y descripción (el mismo de `SidebarBrand`) o un desvío por `onSelect` a un `AppSwitcher` con confirmación. El módulo es opcional y sin él no queda hueco. Plegado queda la marca, con sistema y compañía en el nombre accesible, y sigue abriendo el menú de compañía. La persona no va en el menú: vive arriba a la derecha, en `UserMenu`. Las cinco aplicaciones de ejemplo lo usan en vez de `SidebarBrand`, que sigue existiendo para quien solo tenga una organización y grupos propios.
