...
- **`SidebarIdentity`: sistema, empresa y módulo en la cabecera del menú** (#119). La identidad estaba repartida: la empresa en `SidebarBrand`, el nombre del sistema en un texto suelto de la barra y el módulo, en dos niveles, en el riel. Ahora un bloque en el hueco `brand` con el sistema (marca y nombre, con el distintivo de entorno), la empresa y el módulo, cada uno un menú con marca de selección y descripción (el mismo de `SidebarBrand`) o un desvío por `onSelect` a un `AppSwitcher` con confirmación. El módulo es opcional y sin él no queda hueco. Plegado queda la marca, con sistema y empresa en el nombre accesible, y sigue abriendo el menú de empresa. La persona no va en el menú: vive arriba a la derecha, en `UserMenu`. Las cinco aplicaciones de ejemplo lo usan en vez de `SidebarBrand`, que sigue existiendo para quien solo tenga una organización y grupos propios.
