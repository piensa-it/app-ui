...
- **Naming de controles (controlado/no controlado)**: la regla decide sola, sin mirar al componente hermano — "¿el valor es un booleano de sí/no, o es una selección de un conjunto de opciones?". Booleano de sí/no → `checked`/`onCheckedChange` (Checkbox, Switch) — paridad con el `<input type="checkbox">` nativo, que ya usa `checked`. Selección de un conjunto → `value`/`onChange` (RadioGroup, Slider, Select, MultiSelect, DatePicker, AutoComplete), sin importar el shape del evento que exponga Ark UI por debajo: el wrapper de la librería lo adapta. No "normalizar" Checkbox/Switch a `value`/`onChange` — sería un breaking change sin beneficio real, ya que ambos representan lo mismo que su contraparte HTML nativa. Excepción explícita: `Tabs` usa `onValueChange` pese a ser una selección, porque no es un control de formulario sino navegación — su valor no es un dato del modelo. No la reabras sin releer por qué en `src/version.ts` (entrada 1.0.0).
