...
- **Naming de controles (controlado/no controlado)**: no es arbitrario, sigue el criterio de "¿el valor es un booleano de sí/no, o es una selección de un conjunto de opciones?". Controles binarios (Checkbox, Switch) usan `checked`/`onCheckedChange` — paridad con el `<input type="checkbox">` nativo, que ya usa `checked`. Controles de selección (RadioGroup, Slider, Select, MultiSelect, DatePicker, AutoComplete) usan `value`/`onChange` o `value`/`onValueChange` según el shape del evento que expone Ark UI para ese componente — revisa el componente hermano más parecido antes de inventar un nombre nuevo. No "normalizar" Checkbox/Switch a `value`/`onChange` — sería un breaking change sin beneficio real, ya que ambos representan lo mismo que su contraparte HTML nativa.
