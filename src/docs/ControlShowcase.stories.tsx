...
function ProfessionalForm({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <main className={compact ? "min-h-screen bg-muted p-4" : "min-h-screen bg-muted p-4 sm:p-8 lg:p-12"}>
      <form className={compact ? "mx-auto flex max-w-[375px] flex-col rounded-xl border border-surface-border bg-card p-5 shadow-sm" : "mx-auto flex max-w-3xl flex-col rounded-xl border border-surface-border bg-card p-5 shadow-sm sm:p-8"}>
        {/* El ritmo entre estos tres bloques (cabecera, datos, preferencias)
            es aparte del pie: `gap-8` aquí, no en el `<form>`, para que el pie
            aporte su propio margen vía `actionFooterVariants` (#140) en vez de
            sumarse al del contenedor y quedar con demasiado aire arriba. */}
        ...
        <footer className={cn(actionFooterVariants({ separator: true }), compact && "[&>button]:w-full")}>
